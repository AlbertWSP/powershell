...
$DoSearch = {
    ...
    try {
        ...
        # Searches all groups where user is the primary manager
            ...
            foreach ($group in $groups) { 
                ...
                try {
                    ...
                    foreach($access in $acl.Access) {
                        # Accurate SID translation check
...
$btnUpdate.Add_Click({
    ...
    try {
        $prevOwner = Get-ADUser -Filter "mail -eq '$prevOwnerEmail'" -Properties SID
        $newOwner = Get-ADUser -Filter "mail -eq '$newOwnerEmail'" -Properties SID
        ...
        if ([System.Windows.Forms.MessageBox]::Show($msg, "Confirm", "YesNo") -eq "Yes") {
            ...
            foreach ($item in $checkedItems) {
                ...
                if ($chkRemovePrev.Checked) {
                    $rules = $acl.Access | Where-Object { $_.IdentityReference.Translate([System.Security.Principal.SecurityIdentifier]) -eq $prevOwner.SID }
...
$btnReset.Add_Click({ $inputUser.Text = ""; $inputNewOwner.Text = ""; $outputList.Items.Clear(); $labelList.Text = "Groups Found: 0" })
