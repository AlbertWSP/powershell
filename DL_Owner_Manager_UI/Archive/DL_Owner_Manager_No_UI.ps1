...
function Resolve-ADPrincipal {
    ...
    # Try sAMAccountName / UPN with proper LDAP escaping\n    # Escape special LDAP characters according to RFC 4515\n    $ldapEscapedIdentity = [System.DirectoryServices.Protocols.LdapDefinitions]::EscapeFilterElement($Identity)\n    $filter = \"(&(|(sAMAccountName=$ldapEscapedIdentity)(userPrincipalName=$ldapEscapedIdentity)(name=$ldapEscapedIdentity))(|(objectClass=user)(objectClass=group)(objectClass=contact)))\"
...
function Ensure-ReportPath {
    ...
    # Use user's temp directory instead of hardcoded path
    $outDir = $env:TEMP
...
foreach ($item in $workItems) {
    ...
    try {
        ...
        $ownerSid = New-Object System.Security.Principal.SecurityIdentifier($ownerObj.objectSid)

        # 1) Set ManagedBy
        if ($PSCmdlet.ShouldProcess($group.Name, "Set ManagedBy to $($ownerObj.DistinguishedName)")) {
            Write-Verbose "Setting ManagedBy on group $($group.Name) to $($ownerObj.sAMAccountName)"
        ...
        if (-not $exists) {
            ...
            if ($PSCmdlet.ShouldProcess($group.Name, "Add ACL ACE: Allow WriteProperty on member for $($item.Owner)")) {
                Write-Verbose "Adding ACE to group $($group.Name) for user $($ownerObj.sAMAccountName)"
        ...
        Write-Error "Failed to process $($item.GroupIdentity): $err" -ErrorAction Continue
    }
