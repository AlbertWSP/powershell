...
$tabBatchChange.Controls.Add($dgvResults)
...
$btnViewLog.Location = New-Object System.Drawing.Point(380, 10)
$btnViewLog.Size = New-Object System.Drawing.Size(100, 30)
$btnViewLog.Enabled = $false
$panelBatchButtons.Controls.Add($btnViewLog)
...
$tabSearchGroups.Controls.Add($dgvSearchResults)

# Search Buttons Panel
$panelSearchButtons = New-Object System.Windows.Forms.Panel
$panelSearchButtons.Dock = [System.Windows.Forms.DockStyle]::Bottom
$panelSearchButtons.Height = 50
$panelSearchButtons.BackColor = [System.Drawing.Color]::LightGray
$tabSearchGroups.Controls.Add($panelSearchButtons)

$btnExportSearchReport = New-Object System.Windows.Forms.Button
$btnExportSearchReport.Text = "Export Results"
$btnExportSearchReport.Location = New-Object System.Drawing.Point(10, 10)
$btnExportSearchReport.Size = New-Object System.Drawing.Size(120, 30)
$btnExportSearchReport.Enabled = $false
$panelSearchButtons.Controls.Add($btnExportSearchReport)
...
$btnLoadFile.Add_Click({
    ...
    try {
        ...
        $labelResults.Text = "Loaded: $($emails.Count) groups - Click Preview or Execute to process"
...
$btnPreview.Add_Click({
    ...
    try {
        ...
        # 顯示結果在 DataGridView 中 (重置綁定以確保顯示)
        $dgvResults.DataSource = $null
        $dgvResults.AutoGenerateColumns = $true
        $dgvResults.AutoSizeColumnsMode = [System.Windows.Forms.DataGridViewAutoSizeColumnsMode]::Fill
        $dgvResults.DataSource = $results | Select-Object GroupEmail, GroupName, Owner, Status, Error
        $dgvResults.Refresh()
        $labelResults.Text = "Preview Results: $($results.Count) groups"
...
$btnExecute.Add_Click({
    ...
    # Show Save dialog BEFORE execution
    ...
    try {
        ...
        $dgvResults.AutoGenerateColumns = $true
        $dgvResults.AutoSizeColumnsMode = [System.Windows.Forms.DataGridViewAutoSizeColumnsMode]::Fill
        $dgvResults.DataSource = $script:ProcessingResults | Select-Object GroupEmail, GroupName, Owner, Status, Error
        $dgvResults.Refresh()
...
$btnViewLog.Add_Click({
    if ($script:LastReportPath -and (Test-Path $script:LastReportPath)) {
        Invoke-Item $script:LastReportPath
    }
})

# Search Groups Tab Handlers
$btnSearch.Add_Click({
    ...
    try {
        ...
        if ($results -and $results.Count -gt 0) {
            ...
            $dgvSearchResults.AutoGenerateColumns = $true
            $dgvSearchResults.AutoSizeColumnsMode = [System.Windows.Forms.DataGridViewAutoSizeColumnsMode]::Fill
            $dgvSearchResults.DataSource = $results | Select-Object GroupEmail, GroupName, Owner, Status, Error
            ...
            $btnExportSearchReport.Enabled = $true
        } else {
            $dgvSearchResults.DataSource = $null
...
$btnExportSearchReport.Add_Click({
    if ($dgvSearchResults.DataSource -eq $null -or $dgvSearchResults.Rows.Count -eq 0) {
        [System.Windows.Forms.MessageBox]::Show("No results to export", "Warning", "OK", "Warning")
        return
    }
    ...
    $saveDialog = New-Object System.Windows.Forms.SaveFileDialog
    $saveDialog.Filter = "Text Files (*.txt)|*.txt"
    $saveDialog.FileName = "DL_Search_Results_$(Get-Date -Format 'yyyyMMdd_HHmmss').txt"
    $saveDialog.InitialDirectory = $env:USERPROFILE
    
    if ($saveDialog.ShowDialog() -eq "OK") {
        $searchResults = @()
        foreach ($row in $dgvSearchResults.Rows) {
            $searchResults += [PSCustomObject]@{
                GroupEmail = $row.Cells["GroupEmail"].Value
                GroupName = $row.Cells["GroupName"].Value
                Owner = $row.Cells["Owner"].Value
                Status = $row.Cells["Status"].Value
                Error = $row.Cells["Error"].Value
            }
        }
        Export-OperationReport -FilePath $saveDialog.FileName -Results $searchResults
        [System.Windows.Forms.MessageBox]::Show("Exported: $($saveDialog.FileName)", "Success", "OK", "Information")
    }
})
