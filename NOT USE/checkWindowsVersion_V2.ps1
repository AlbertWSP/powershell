...
$computerListPath = "C:\temp\hostname.txt"
...
foreach ($computerName in $computerNames) {
    try {
        $computer = Get-ADComputer -Identity $computerName -Properties Name, OperatingSystem, OperatingSystemVersion, IPv4Address, LastLogonDate -ErrorAction Stop
        if ($computer) {
            $errorStatus = "None"
            try {
                # Test connection to verify if computer is online
                $testConnection = Test-Connection -ComputerName $computerName -Count 1 -ErrorAction Stop
...
$csvPath = "C:\temp\Output_ALlcomputer_win11_$timestamp.csv"
