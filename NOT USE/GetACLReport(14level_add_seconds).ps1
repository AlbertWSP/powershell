...
#Author: Kin.Yan@wsp.com <----Chief Football Officer(CFO) of Guangzhou, China office. ^_^
#Description: Believe it or not, you will never seen a shell better than this one, powerful, user friendly, beautiful UI(I said beautiful, it's beautiful)
#             generate ACL report becomes sooooo much easy. It's AMAZING!!!! (applaud~)

#What's new in V2.1(2022/7/12):
#1. You are able to generate a report to any folder path
#2. Add one more column(DisplayName) to the report
#3. Update scan folder list(HK T&I)
###

#What's new in V2.0(2022/6/16):
#1. Scan folder update(HK MEP1)
###

#What's new in V2.0(2022/6/13):
#1. You are able to control the depth of ACL report to the target share folder.
###

#What's new in V1.0(2022/5/13):
#1. Scan folder update(HK SQA)
###

#What's new in V1.0(2022/4/26):
#1. Scan folder update(GZ/SZ/CD)
###

#What's new in V1.0(2022/2/24):
#1. Initial release
###

<#
param([Parameter(Mandatory)]$Office,
	$Team
	)
#>
$ShellVersion="2.1"
$AclFolderVersion="20220713"
$FormTitle="Access Control List Report Master Pro Ultimate - V" + $ShellVersion
$PowershellPath = "C:\"
$ReportPath = Join-Path $PowershellPath "temp"


$ACLFolders=@(	
	("Beijing(CNBJS300)","MEP","\\cnbjs300dat01\PROJECT"),
	("Beijing(CNBJS300)","Admin","\\cnbjs300dat02\ADMIN"),
	("Beijing(CNBJS300)","BD","\\cnbjs300dat02\BD"),
	("Beijing(CNBJS300)","CD","\\cnbjs300dat02\CD"),
	("Beijing(CNBJS300)","Common","\\cnbjs300dat02\Common"),
	("Beijing(CNBJS300)","FA","\\cnbjs300dat02\FA"),
	("Beijing(CNBJS300)","HR","\\cnbjs300dat02\HR"),
	("Beijing(CNBJS300)","IT","\\cnbjs300dat02\IT"),
	("Beijing(CNBJS300)","Transport","\\cnbjs300dat02\Transport"),
        ...
        for($i=0;$i -lt $Identitys.count;$i++)
            {
                ...
                if($Identity.Value.Substring(0,5) -eq "CORP\")
                {
                    ...
                    if($IdentityName -eq "GRP-FSS-CNGGZ100-IT")
                    {
                        $a=1
                    }
                    ...
                    {
                        $GroupMembers=Get-ADGroupMember $IdentityName -recursive | select SamAccountName
					}
...
$PowerShellForms.Size = New-Object System.Drawing.Size(500,500)
...
$LbDepth.Location = New-Object System.Drawing.Point(325,389)
...
$TbDepth.Location = New-Object System.Drawing.Point(442,385)
...
$LbTargetPath.Location = New-Object System.Drawing.Point(15,300)
...
$TbTargetPath.Location = New-Object System.Drawing.Point(110,300)
$TbTargetPath.Size = New-Object System.Drawing.Size(360,30)
...
$BtnOK.Location = New-Object System.Drawing.Size(323,420)
$BtnOK.Text = "Get it!"
$BtnOK.Font = New-Object System.Drawing.Font("Tahoma",10,[System.Drawing.FontStyle]::Regular)
$BtnOK.Add_Click(
    {
        #[System.Windows.Forms.Messagebox]::Show($TabControl1.SelectedIndex)
        ...
        if($TabControl1.SelectedIndex -eq 0)
        {
            if((($CbAllTeam.Checked -eq $true) -and ($null -ne $OfficeList.SelectedItem)) -or (($null -ne $OfficeList.SelectedItem) -and ($null -ne $TeamList.SelectedItem)))
            {
			    ...
			    #[System.Windows.Forms.Messagebox]::Show($OfficeList.SelectedItem)
			    $SelectedOffice=$OfficeList.SelectedItem
			    $SelectedTeam=$TeamList.SelectedItem
			    #[System.Windows.Forms.Messagebox]::Show($null -eq $SelectedTeam)
			    ...
			    for($n=0;$n -lt $ACLFolders.Length;$n++)
			    {
				    if($ACLFolders[$n][0] -eq $SelectedOffice)
				    {
					    ...
					    $reportname = "ACLReport_" + $SelectedOffice + "_" + $ACLFolders[$n][1] + "_" + $today + ".csv"
					    ...
					    {
						    $R=Get-ACLReport $ACLFolders[$n][2]
						    $R | export-csv $reportname -NoTypeInformation -Force -Append -Encoding UTF8

						    if($TbDepth.Text -eq "*")
						    {
							    $SubFolders=Get-ChildItem -path $ACLFolders[$n][2] -Directory -Force -Recurse | Select-Object FullName
						    }
						    else
						    {
							    ...
							    $SubFolders=Get-ChildItem -path $ACLFolders[$n][2] -Depth $Depth -Directory -Force | Select-Object FullName
						    }
        ...
        else
        {
            ...
            {
					    $reportname = "ACLReport_" + $today + ".csv"
					    $reportname = Join-Path $ReportPath $reportname
...
$tabPage1.text = "Have Fun"
$tabPage1.TabIndex=0
$tabPage1.Controls.AddRange(($Lboffice, $OfficeList, $LbTeam, $TeamList, $LbAllTeam,$CbAllTeam))

$tabPage2=New-Object System.Windows.Forms.TabPage
$tabPage2.text = "Be Happy"
$tabPage2.TabIndex=1
$tabPage2.Controls.AddRange(($LbTargetPath,$TbTargetPath))
...
$Tabcontrol1.Add_SelectedIndexChanged(
    {
        $TabControl1.TabPages[0].Controls.AddRange(($Lboffice, $OfficeList, $LbTeam, $TeamList, $LbAllTeam,$CbAllTeam,$LbDepth,$TbDepth,$BtnOK,$LbTargetPath,$TbTargetPath,$LbDepth,$TbDepth,$BtnOK2))
    }
)
#>
