...
function Show-FileSelectionDialog {
    ...
    # Add concurrent jobs control
    $concurrentLabel = New-Object System.Windows.Forms.Label
    $concurrentLabel.Text = "Number of concurrent scans:"
    $concurrentLabel.AutoSize = $true
    $concurrentLabel.Location = New-Object System.Drawing.Point(10, 130)
    $form.Controls.Add($concurrentLabel)

    $concurrentNumeric = New-Object System.Windows.Forms.NumericUpDown
    $concurrentNumeric.Location = New-Object System.Drawing.Point(10, 150)
    $concurrentNumeric.Size = New-Object System.Drawing.Size(100, 20)
    $concurrentNumeric.Minimum = 1
    $concurrentNumeric.Maximum = 10
    $concurrentNumeric.Value = 2
    $form.Controls.Add($concurrentNumeric)
    ...
    $button.Add_Click({
        ...
        if (Test-Path $pathsFile) {
            Process-Folders $pathsFile $depthNumeric.Value $concurrentNumeric.Value
            $form.Close()
        }
        else {
...
function Process-Folders ($pathsFile, $depth, $maxConcurrent) {
    ...
    foreach ($folder in $folders) {
        if (Test-Path $folder) {
            ...
            $jobs += Start-Job -ScriptBlock {
                ...
                # Include the Get-ACLReport function in the job
                function Get-ACLReport ($folder, $depth) {
                    $report = @()
                    try {
                        # Get folders based on specified depth
