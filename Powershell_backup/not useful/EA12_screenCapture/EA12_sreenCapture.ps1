...
foreach ($email in $emailAddresses) {
    try {
        ...
        if ($obj.objectClass -eq "user") {
            ...
            get-aduser -identity $obj.samaccountname -Properties * | Select-Object DisplayName, @{Label="LastName";Expression={$_.sn}}, @{Label="FirstName";Expression={$_.GivenName}}, 
            sAMAccountName, @{Label="UPN Logon";Expression={$_.userPrincipalName}}, @{Label="FullName";Expression={$_.Name}}, 
            Mail, Description, @{Label="ExpireDate";Expression={[datetime]::FromFileTime($_.accountExpires)}}, Enabled, 
            ...
            Start-Sleep -Seconds 1
            TakeScreenshot -ObjectID -ObjectID $email -Left $captureLeft -Top $captureTop -Width $captureWidth -Height $captureHeight
