...
function Process-Folders ($pathsFile) {
    ...
    foreach ($folder in $folders) {
        if (Test-Path $folder) {
            ...
            # Create combined results for this folder
            $combinedResults = @()
            foreach ($currentFolder in $allFolders) {
                Write-Host "Scanning size for: $currentFolder" -ForegroundColor Gray
                $folderStats = Get-FolderStats $currentFolder
                
                # Get ACL results for this folder
                $folderAclResults = $aclResults | Where-Object { $_.FolderPath -eq $currentFolder }
                
                if ($folderAclResults.Count -gt 0) {
                    foreach ($aclResult in $folderAclResults) {
                        $combinedResult = [PSCustomObject]@{
                            FolderPath = $currentFolder
                            FolderSize = Format-FileSize -Size $folderStats.Size
                    ...
                    # If no ACL results found, still add the folder with size info
                    $combinedResult = [PSCustomObject]@{
                        FolderPath = $currentFolder
                        FolderSize = Format-FileSize -Size $folderStats.Size
                        FileCount = $folderStats.FileCount
                        Identity = "No ACL data"
                        Member = "-"
                        DisplayName = "-"
                        AccessRight = "-"
                        Owner = "-"
                ...
                Write-Host (Format-FileSize -Size $folderStats.Size) -ForegroundColor (Get-SizeColor -Size $folderStats.Size)
                Write-Host "Files: " -NoNewline
                Write-Host $folderStats.FileCount -ForegroundColor Magenta
                Write-Host "-------------------"
            }
            
            # Export results for this folder
            $today = Get-Date -Format "yyyyMMddHHmm"
            $reportName = Join-Path $ReportPath "Report_${folderName}_$today.csv"
