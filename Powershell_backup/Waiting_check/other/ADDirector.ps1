...
Version 0.2 ()
New:
1. In case duplicate default groups are preset in the Teams.xml, only unique groups will be added to the member group list
2. Validate if target groups are in AD or not before adding to member group list
3. Add user Title and Tel(Home) filed. Tel(Home) is specificly for TW.
Bug fix: 
...
$Page1_TBJoinerSamAccountName.Add_LostFocus(
    {
        if(($Page1_RBFinalizeADAccount.Checked -eq $True) -and ($Page1_TBJoinerSamAccountName.Text -ne ""))
        {
                ...
                try {
                    ...
                    if($userInfo.extensionAttribute11 -eq "")
                    {
                        $Page1_TBJoinerEA11.Text = "PERSONAL"
                    ...
                    if($userInfo.employeeType -eq "")
                    {
                        $Page1_LbJoinerEmployeeType.Text = "PERSONAL"
                        $Page1_LbJoinerEmployeeType.ForeColor = "Red"
                    }
                    else {
                        $Page1_LbJoinerEmployeeType.Text = $userInfo.employeeType
                    }
            ...
            if($xmldata.Sites.ChildNodes[$site].Code -eq $Page1_OfficeList.SelectedItem)
            {
                ...
                foreach($Department in $xmldata.Sites.ChildNodes[$site].ChildNodes)
                {
                    if($Department.Name -eq $Page1_DepartmentList.SelectedItem)
                    {
                        ...
                        #Add logon script
                        if($Page1_TBJoinerLogonScript.Text -ne $Department.LogonScript)
                        {
...
$Page1_BtnAddGroup.Add_Click(
    {
        ...
        foreach($TargetGroup in $TargetGroups)
        {
            ...
            {
                $groupcount=@(Get-ADGroup $TargetGroup.Trim()).Count
                if($groupcount -gt 0)
                {
                    ...
                    if(($Result -eq "OK") -or ($EmailConflict -eq $false))
                    {
                        ...
                        try {
                            ...
                            try {
                                foreach($ADGroup in $Page1_DefaultGroupList.Items)
                                {
                                    get-aduser -Identity $Page1_TBJoinerSamAccountName.Text | %{Add-ADGroupMember -Identity $ADGroup -Members $_.samaccountname} -ErrorAction Continue
                                }
                            }
                            catch {
