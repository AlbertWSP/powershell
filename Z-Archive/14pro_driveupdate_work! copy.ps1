...
# ══════════════════════════════════════════════════════════════════════════════
#  Setup File Deployment v5.1 – Bug Fixes + Local Log Collection
#  Supports Dell Update Packages (DUP) with /s /l /r /p switches
#  Changelog v5.1:
#    - [FIX] UInt32 conversion error for negative exit codes (e.g. 0xC0000409)
#    - [FIX] Added NTSTATUS crash code lookup (STATUS_STACK_BUFFER_OVERRUN, etc.)
#    - [NEW] Logs collected from remote PCs back to local folder after install
#    - [NEW] Local log folder: C:\\temp\\DeploymentLogs\\<timestamp>\\
#    - [NEW] Remote logs renamed with hostname prefix for easy identification
#    - [NEW] Auto-export DeploymentSummary.csv into log folder
#    - Carries forward all v5 features (progress bar, log panel, stop, CSV, etc.)
# ══════════════════════════════════════════════════════════════════════════════
...
$script:ntStatusMap = @{
    ...
    "C000013A" = "STATUS_CONTROL_C_EXIT (process terminated)"
    "C0000142" = "STATUS_DLL_INIT_FAILED"
    "C0000374" = "STATUS_HEAP_CORRUPTION"
    "C0000409" = "STATUS_STACK_BUFFER_OVERRUN"
    "C06D007E" = "STATUS_MOD_NOT_FOUND"
    "C06D007F" = "STATUS_PROC_NOT_FOUND"
}
...
# ── Helper: Interpret exit code (Dell DUP + Windows NTSTATUS) ─────────────────
function Get-ExitCodeDescription {
    param([int]$Code)

    # Check Dell DUP exit codes first
    if ($script:exitCodeMap.ContainsKey($Code)) {
        return $script:exitCodeMap[$Code]
    }

    # For negative codes, convert to hex via two's complement and check NTSTATUS
    if ($Code -lt 0) {
        $hexStr  = ([int32]$Code).ToString("X8")
        $hexCode = "0x$hexStr"

        if ($script:ntStatusMap.ContainsKey($hexStr)) {
            return "$($script:ntStatusMap[$hexStr]) ($hexCode)"
        }
        return "APPLICATION CRASH (Exit Code: $Code / $hexCode)"
    }
...
$form.Text            = "Setup File Deployment v5.1"
...
$btnExportCsv.Add_Click({
    ...
    if ($script:localLogFolder -and (Test-Path $script:localLogFolder)) {
        $saveDialog.InitialDirectory = $script:localLogFolder
    } else {
...
$okButton.Add_Click({
    ...
    $script:localLogFolder = "C:\\temp\\DeploymentLogs\\$timestamp"
    try {
        New-Item -ItemType Directory -Path $script:localLogFolder -Force | Out-Null
    }
    catch {
        [System.Windows.Forms.MessageBox]::Show(
            "Cannot create local log folder:`n$($script:localLogFolder)`n$($_.Exception.Message)",
            "Log Folder Error",
            [System.Windows.Forms.MessageBoxButtons]::OK,
            [System.Windows.Forms.MessageBoxIcon]::Warning
        )
        $script:localLogFolder = "C:\\temp"
    }
    ...
    Write-Log $richLog "  Setup File Deployment v5.1" ([System.Drawing.Color]::Cyan)
    Write-Log $richLog "=================================================" ([System.Drawing.Color]::Cyan)
    ...
    Write-Log $richLog "  Logs   : $($script:localLogFolder)" ([System.Drawing.Color]::Cyan)
    ...
    foreach ($name in $hostNames) {
        ...
        foreach ($label in $deployments.Keys) {
            ...
            try {
                ...
                Write-Log $richLog "  [$taskNum/$totalTasks][$($pkg.FileName)] Copy... OK" ([System.Drawing.Color]::FromArgb(0, 200, 0))
            }
            catch {
                Write-Log $richLog "  [$taskNum/$totalTasks][$($pkg.FileName)] Copy... FAILED: $($_.Exception.Message)" ([System.Drawing.Color]::Red)
                $failCount++
                $script:deployResults += [PSCustomObject]@{ Host=$name; Package=$($pkg.FileName); Step="Copy"; Status="FAILED" }
                $progressBar.Value = [Math]::Min($taskNum, $totalTasks)
                $labelPercent.Text = "$pct%"
                [System.Windows.Forms.Application]::DoEvents()
                continue
            ...
            if ($doInstall) {
                if (-not $winrmOk) {
                    Write-Log $richLog "  [$taskNum/$totalTasks][$($pkg.FileName)] Install SKIPPED (WinRM unavailable)" ([System.Drawing.Color]::Yellow)
                    $failCount++
                    $script:deployResults += [PSCustomObject]@{ Host=$name; Package=$($pkg.FileName); Step="Install"; Status="SKIPPED (WinRM unavailable)" }
                    $progressBar.Value = [Math]::Min($taskNum, $totalTasks)
                    $labelPercent.Text = "$pct%"
                    [System.Windows.Forms.Application]::DoEvents()
                    continue
                ...
                try {
                    ...
                    # ── Interpret exit code (fixed: no UInt32 crash) ──────
                    $exitDesc = Get-ExitCodeDescription -Code ([int]$exitCode)
                    ...
                    if ($exitCode -eq 0 -or $exitCode -eq 2 -or $exitCode -eq 6) {
                        ...
                        Write-Log $richLog "  [$taskNum/$totalTasks][$($pkg.FileName)] $exitDesc" $clr
                        $successCount++
                    } else {
                        Write-Log $richLog "  [$taskNum/$totalTasks][$($pkg.FileName)] $exitDesc" ([System.Drawing.Color]::Red)
                        $failCount++
                    }
                    $script:deployResults += [PSCustomObject]@{ Host=$name; Package=$($pkg.FileName); Step="Install"; Status=$exitDesc }
                }
                catch {
                    Write-Log $richLog "  [$taskNum/$totalTasks][$($pkg.FileName)] ERROR: $($_.Exception.Message)" ([System.Drawing.Color]::Red)
                    $failCount++
                    $script:deployResults += [PSCustomObject]@{ Host=$name; Package=$($pkg.FileName); Step="Install"; Status="ERROR: $($_.Exception.Message)" }
                }

                # ── Collect remote log back to local PC ───────────────────
                $remoteLogUNC = "\\\\$name\\C`$\\temp\\$($pkg.LogName)"
                $localLogName = "${name}_$($pkg.LogName)"
                $localLogPath = Join-Path $script:localLogFolder $localLogName

                Update-Progress $progressBar $labelStatus $taskNum $totalTasks "[$taskNum/$totalTasks] Collecting log from $name..."
                try {
                    if (Test-Path -Path $remoteLogUNC -ErrorAction SilentlyContinue) {
                        Copy-Item -Path $remoteLogUNC -Destination $localLogPath -Force -ErrorAction Stop
                        Write-Log $richLog "  [$taskNum/$totalTasks][$($pkg.FileName)] Log collected: $localLogName" ([System.Drawing.Color]::Gray)
                        $logsCopied++
                    } else {
                        Write-Log $richLog "  [$taskNum/$totalTasks][$($pkg.FileName)] No remote log found to collect." ([System.Drawing.Color]::DarkGray)
                    }
                }
                catch {
                    Write-Log $richLog "  [$taskNum/$totalTasks][$($pkg.FileName)] Log collect failed: $($_.Exception.Message)" ([System.Drawing.Color]::DarkGray)
                }
            }
            else {
                $successCount++
                $script:deployResults += [PSCustomObject]@{ Host=$name; Package=$($pkg.FileName); Step="Copy"; Status="COPIED" }
            }
    ...
    if ($doInstall) {
        Write-Log $richLog "" ([System.Drawing.Color]::White)
        Write-Log $richLog "Install logs collected: $logsCopied file(s)" ([System.Drawing.Color]::Gray)
        Write-Log $richLog "Local log folder: $($script:localLogFolder)" ([System.Drawing.Color]::Gray)
    }

    # ── Auto-export CSV to log folder ──────────────────────────────────────
    if ($script:deployResults.Count -gt 0) {
        $csvAutoPath = Join-Path $script:localLogFolder "DeploymentSummary.csv"
        try {
            $script:deployResults | Export-Csv -Path $csvAutoPath -NoTypeInformation -Encoding UTF8
            Write-Log $richLog "Summary CSV: $csvAutoPath" ([System.Drawing.Color]::Gray)
        }
        catch {
            Write-Log $richLog "Auto CSV export failed: $($_.Exception.Message)" ([System.Drawing.Color]::DarkGray)
        }
    }
