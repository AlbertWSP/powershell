...
function Remove-RemoteSoftware {
    ...
    try {
        $targetSoftware = $SoftwareList | Where-Object { $_.Name -eq $SoftwareName }
        
        if ($targetSoftware) {
            Write-Host "Attempting to uninstall $SoftwareName..." -ForegroundColor Yellow
            
            $app = Get-WmiObject -Class Win32_Product -ComputerName $ComputerName |
                   Where-Object { $_.Name -eq $SoftwareName }
            
            if ($app) {
                $result = $app.Uninstall()
                if ($result.ReturnValue -eq 0) {
                    Write-Host "Software uninstallation completed successfully!" -ForegroundColor Green
                    return $true
                } else {
            ...
            Write-Host "Software '$SoftwareName' not found in the installed software list." -ForegroundColor Red
            return $false
        }
    }
    catch {
...
try {
    ...
    do {
        ...
        Write-Host "2. Remove software directly" -ForegroundColor Yellow
        ...
        switch ($choice) {
            "1" {
                ...
                if ($softwareList) {
                    $remove = Read-Host "`nWould you like to remove any software? (Y/N)"
                    if ($remove -match '^[Yy]') {
                        $softwareToRemove = Read-Host "Enter the exact name of the software to remove"
                        Remove-RemoteSoftware -ComputerName $remotePCName -SoftwareName $softwareToRemove -SoftwareList $softwareList
                    }
                }
            }
            "2" {
                # Direct software removal
                $softwareToRemove = Read-Host "Enter the exact name of the software to remove"
                # Get minimal software list for verification
                $softwareList = Get-WmiObject -Class Win32_Product -ComputerName $remotePCName | 
                               Where-Object { $_.Name -eq $softwareToRemove }
                if ($softwareList) {
                    Remove-RemoteSoftware -ComputerName $remotePCName -SoftwareName $softwareToRemove -SoftwareList $softwareList
                } else {
                    Write-Host "Software '$softwareToRemove' not found on the remote computer." -ForegroundColor Red
                }
            }
            "3" {
        ...
        $continue = Read-Host "`nWould you like to perform another operation? (Y/N)"
