...
foreach ($folder in $folders) {
    # Get the size of the folder and the number of files and folders
    $items = Get-ChildItem -Path $folder -Recurse
    $folderSize = ($items | Measure-Object -Property Length -Sum).Sum
    $fileCount = ($items | Where-Object { !$_.PSIsContainer } | Measure-Object).Count
...
$results | Export-Csv -Path "C:\temp\Folders_output_H503.csv" -NoTypeInformation

Write-Host "Results exported to C:\temp\Folders_output_H503.csv" -ForegroundColor Green