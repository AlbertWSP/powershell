...
$setupFilePath1 = "\\corp.pbwan.net\hk\Transition\DAT18\kits\Software\Engineering\ETABS\ETABSv2270Setup.exe"
...
$destinationPath1 = "\\$hostName\C$\temp\ETABSv2270Setup.exe"
...
if ($option1Enabled) {
    Perform-Setup -hostName $hostName -setupFilePath $setupFilePath1 -destinationPath $destinationPath1 -setpath "C:\temp\ETABSv2270Setup.exe" -commandLine '/s /v"/qn LicenseMode=Login"'
}
if ($option2Enabled) {
