...
write-host "`n####### AD Object(DL/SG/User) Info Collection Tool V1.3 #######`n"
...
if($obj.objectClass -eq "user")
{
	...
	Get-ADPrincipalGroupMembership -Identity $obj.samaccountname | select @{Label="Member of";Expression={$_.name}} | format-wide -autosize
    write-host "`n####### End #######`n"
}
