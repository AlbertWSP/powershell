...
# =================================================================================
# 3. UI Interface Code (Version 2.7- Improved Edition)
# =================================================================================

$form = New-Object System.Windows.Forms.Form
$form.Text = "AD Security Group Manager - Version 2.6"
...
$btnCheckAcl.Add_Click({
    ...
    if (Test-Path $path) {
        ...
        try {
            ...
            # BEGIN CHANGE - Enable Excel and CSV export after ACL load
...
$btnExportAclExcel.Add_Click({
    ...
    if ($saveFile.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
        ...
        try {
            ...
            [System.Windows.Forms.MessageBox]::Show("ACL data exported successfully to:`n`n$($saveFile.FileName)","Export Complete","OK","Information")
            Update-Status "ACL Excel export completed: $($saveFile.FileName)"
        }
        catch {
            [System.Windows.Forms.MessageBox]::Show("Excel export failed.`n`n$($_.Exception.Message)","Export Error","OK","Error")
...
$btnExportAclCsv.Add_Click({
    ...
    if ($saveFile.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
        ...
        try {
            $script:currentAclData | Select-Object IdentityReference, FileSystemRights, AccessControlType | Export-Csv -Path $saveFile.FileName -NoTypeInformation -Encoding UTF8 -Force
            [System.Windows.Forms.MessageBox]::Show("ACL data exported successfully to:`n`n$($saveFile.FileName)", "Export Complete", "OK", "Information")
            Update-Status "ACL CSV export completed."
        } catch {
...
$txtAccessPath.Anchor = "Top, Left, Right"
...
$txtGroupSearch.Anchor = "Top, Left, Right"
...
$txtTargetGroup.Anchor = "Top, Left, Right"
            ...
            if ($gName -in $userGroups -and $gName -notin $matchedGroupNames) {
                ...
                try {
                    ...
                    if ($userSam -notin $directSams) {
                        # Optimized: Avoid enumerating every recursive sub-group member to prevent UI freeze
                        $membershipPath = "Nested Member (Indirect)"
                    }
